...
1. 下の表からファイルを 1 つ選ぶ
2. **ファイルを丸ごとコピー** (中身全部)
3. **必要な入力画像を添付** (各プロンプトの `[入力画像]` 欄に書いてある)
4. ChatGPT に貼り付けて画像を生成 → 指定の保存先に保存
...
```mermaid
flowchart TB
    subgraph S0["ステップ 0: マスター画像 (1 回)"]
        FM[frame-master.png]
        RM[reference-master.png]
    end

    subgraph S1["ステップ 1: リファレンス画像 (1 回)"]
        Char[キャラ立ち絵 6 体]
        Lab[ラボ内装]
        Style[スタイルガイド]
    end

    subgraph S2["ステップ 2: フレーム画像 (1 回)"]
        F1[4koma / 6koma など]
    end

    subgraph S3["ステップ 3: エピソード生成 (毎話)"]
        Ep[各話 cover.png]
    end
...
**マスター画像が "トンマナの基準" になり、配色 / 線 / 装飾の語彙が
全ファイル間でブレないようにする** のが今の運用方針。
...
**入力画像として `reference-master.png` を必ず添付** する。
...
**入力画像として `frame-master.png` を必ず添付** する。
...
> マスター画像 (`frame-master.png` / `reference-master.png`) は不要。
> 個別フレームと個別キャラ refs に既にトンマナが反映されているため。
