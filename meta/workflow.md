...
- ChatGPT images で生成する場合は [`./image-prompts/`](./image-prompts/) のプロンプト集を使う
  - 各ファイルは **丸ごとコピペで使える** 自己完結プロンプト
  - **マスター画像** (frame-master / reference-master) を最初に 1 回だけ生成 (トンマナ基準)
  - リファレンス画像 (キャラ立ち絵 / ラボ内装) は **初回のみ** 生成して `assets/` に保存
  - フレーム画像 (コマ割りテンプレ) も **初回のみ** 生成して `assets/concept/frames/` に保存
  - 各エピソードは [`./image-prompts/episode/composition.md`](./image-prompts/episode/composition.md) のシナリオ部分を埋めて使う
