...
| 時期       | 出来事                                                                          |
| ---------- | ------------------------------------------------------------------------------- |
| `Y-数年`    | シュンタ、ある程度大きなチームに在籍。やりがいはあるが "段取り疲れ" を感じ始める |
| `Y-2` 前後 | シュンタ、家族として **モナカ**（ゴールデンドゥードゥル）を迎える。朝晩の散歩が生活リズムの柱になる |
| `Y-1` 前後 | シュンタが副業案件をいくつか回す中で **ミカ** と知り合う                         |
| `Y-1` 前後 | シュンタ、前職の先輩 **タクマ** と継続的に飲み仲間として近い距離感を保つ           |

### 設立 (`Y0`)

| 時期       | 出来事                                                                |
| ---------- | --------------------------------------------------------------------- |
| `Y0` 春     | シュンタ、週末プロジェクトの延長として **デジラボ設立**。一人で開始    |
| `Y0` 春     | モナカ、シュンタと一緒に "通勤" するようになり、自然にラボの一員になる |
| `Y0` 夏     | "話しかけられる相棒が欲しい" と感じ、抽象 AI **アイリス** を常駐させる |
| `Y0` 秋     | ミカ、休日に何度か顔を出すうちに **デザイン担当として参画**          |
| `Y0` 冬     | タクマ、"出さないと意味ない" と言いに来てそのまま **居着く**          |

> アイリスは "ある日突然導入された" のではなく、**シュンタが試行錯誤しながら
...
| 時期       | 出来事                                                                     |
| ---------- | -------------------------------------------------------------------------- |
| `Y+1` 春   | デジラボの 4 人 + 1 体 + 1 匹体制が安定する                                 |
| `Y+1` 春   | シュンタが「ものづくりの裏側を漫画で残してみよう」と決める → **連載開始**   |
